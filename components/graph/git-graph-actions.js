...
class Squash extends ActionBase {
  ...
  perform() {
    ...
    if (path.length > 0) {
      ...
      //  \           ->     \
      //   d  - <e>           d - <e>
      const commitMessage = `Squash merge from ${this.node.sha1.substring(0, 8)} into ${onto.sha1.substring(0, 8)}`;
      ...
      //                ->     \
      //                        [bc]
      const commitMessage = `Squash merge from ${onto.sha1.substring(0, 8)} into ${this.node.sha1.substring(0, 8)}`;
      return this.graph
        .currentActionContext()
        .moveTo(this.node.sha1, true)
        .then(() =>
          this.server.postPromise('/squash', { 
            path: this.graph.repoPath(), 
            target: onto.sha1,
            message: commitMessage,
          })
