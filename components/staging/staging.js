...
class StagingViewModel extends ComponentRoot {
  constructor(server, repoPath, graph) {
    ...
    this.nUnstagedFiles = ko.computed(() => {
      const count = this.files().filter((f) => f.editState() === 'none').length;
      // Update window title with unstaged file count
      if (typeof ungit !== 'undefined' && ungit.windowTitle) {
        ungit.windowTitle.unstagedCount = count;
        ungit.windowTitle.update();
      }
      return count;
    });
